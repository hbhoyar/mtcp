...
/*----------------------------------------------------------------------------*/
static int sendMsg(struct thread_context *ctx, int sockid) {
    char *msg = "Hello I am server";
    int ret = mtcp_write(ctx->mctx, sockid, msg, strlen(msg));
    if (ret < 0) {
...
/*----------------------------------------------------------------------------*/
static int readMsg(struct thread_context *ctx, int sockid) {
    char buf[BUFSIZE];
    char response[BUFSIZE];

    int rd = mtcp_read(ctx->mctx, sockid, buf, BUFSIZE);
    if (rd <= 0) {
        return rd;
    }

    memcpy(response, buf, rd);
    TRACE_APP("Socket %d HTTP Response: \n%s", sockid, response);
    int sent = mtcp_write(ctx->mctx, sockid, response, rd);
    ...
    while (!done[core]) {
        ...
        for (i = 0; i < nevents; i++) {
            if (events[i].data.sockid == listener) {
                ...
                ret = readMsg(ctx, events[i].data.sockid);
...
static void printHelp(const char *prog_name) {
    TRACE_CONFIG(
        "%s [-p <port> (5000)] -f <mtcp_conf_file> "
